    ...
    >
      ...
      >
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <DossierPiecesSection submissionId={submission.id} />
...
function DossierPiecesSection({ submissionId }: { submissionId: string }) {
  const [loading, setLoading] = useState(true)
  ...
  const load = async () => {
    ...
    try {
      const headers = await authHeaders()
      const res = await fetch(`${API}/api/bceg/admin/submissions/${submissionId}/documents`, { headers })
      const json = await res.json()
      if (json?.success) { setDocs(json.documents || []); setChecklist(json.checklist || []) }
      else setErr(json?.error || 'Erreur chargement pièces')
  ...
  const review = async (docId: string, status: 'approved' | 'rejected') => {
    ...
    if (status === 'rejected') {
      const r = window.prompt('Motif du rejet (visible par le porteur) :', '')
      if (r === null) return
      rejection_reason = r
    }
    setActing(docId); setErr(null)
    try {
      const headers = await authHeaders()
      const res = await fetch(`${API}/api/bceg/admin/due-diligence/${docId}`, {
        method: 'PATCH', headers,
        body: JSON.stringify({ status, ...(rejection_reason !== undefined ? { rejection_reason } : {}) }),
      })
      const json = await res.json()
  ...
  const LABELS: Record<string, string> = {
    business_plan: 'Business Plan', plan_action: "Plan d'action", cni: 'CNI', rccm: 'RCCM', rib: 'RIB', devis: 'Devis',
  }
  const statusChip = (s: string | null) => {
    if (s === 'approved') return <span className="text-emerald-700 inline-flex items-center gap-1"><CheckCircle2 className="w-3 h-3" />Validée</span>
    if (s === 'rejected') return <span className="text-rose-700 inline-flex items-center gap-1"><XCircle className="w-3 h-3" />Rejetée</span>
    return <span className="text-amber-700 inline-flex items-center gap-1"><Clock className="w-3 h-3" />À examiner</span>
  }

  return (
    <Section title="Pièces du dossier" icon="📎">
      {loading ? (
        <div className="flex items-center gap-2 text-sm text-slate-500"><Loader2 className="w-4 h-4 animate-spin" />Chargement des pièces…</div>
      ) : (
        <>
          {/* Checklist de complétude */}
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-3">
            {checklist.map((c: any) => (
              <div key={c.key} className={`text-xs rounded-lg border px-2 py-1.5 flex items-center gap-1.5 ${c.provided ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : c.required ? 'border-rose-200 bg-rose-50 text-rose-700' : 'border-slate-200 bg-slate-50 text-slate-500'}`}>
                {c.provided ? <CheckCircle2 className="w-3.5 h-3.5 shrink-0" /> : <XCircle className="w-3.5 h-3.5 shrink-0" />}
                <span className="truncate">{c.label}{!c.required && ' (opt.)'}</span>
              </div>
            ))}
          </div>

          {docs.length === 0 ? (
            <p className="text-sm text-slate-500 italic">Aucune pièce téléversée pour ce dossier.</p>
          ) : (
            <ul className="space-y-2">
              {docs.map((d: any) => (
                <li key={d.id} className="flex items-center justify-between gap-3 border border-slate-200 rounded-lg px-3 py-2">
                  <div className="min-w-0">
                    <div className="text-sm font-semibold text-slate-800 truncate">
                      {LABELS[d.doc_type] || d.doc_type} <span className="text-slate-400 font-normal">— {d.file_name}</span>
                    </div>
                    <div className="text-[11px] mt-0.5">
                      {statusChip(d.verification_status)}
                      {d.rejection_reason ? <span className="text-rose-600"> · {d.rejection_reason}</span> : null}
                    </div>
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    {d.view_url && (
                      <a href={d.view_url} target="_blank" rel="noreferrer" className="p-1.5 rounded hover:bg-slate-100 text-slate-500" title="Voir la pièce"><Eye className="w-4 h-4" /></a>
                    )}
                    <button disabled={acting === d.id} onClick={() => review(d.id, 'approved')} className="p-1.5 rounded hover:bg-emerald-50 text-emerald-600 disabled:opacity-50" title="Valider">
                      {acting === d.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle2 className="w-4 h-4" />}
                    </button>
                    <button disabled={acting === d.id} onClick={() => review(d.id, 'rejected')} className="p-1.5 rounded hover:bg-rose-50 text-rose-600 disabled:opacity-50" title="Rejeter">
