...
export default function BcegSimulator({
  ...
  return (
    <div className={`bg-white/5 backdrop-blur-md border border-white/15 rounded-2xl ${compact ? 'p-4' : 'p-5 sm:p-6'}`}>
      {/* Header */}
      <div className="flex items-center justify-between gap-2 mb-4">
        <div className="flex items-center gap-2">
          <Wallet className="w-5 h-5 text-amber-300" />
          <h3 className="font-bold text-white text-base sm:text-lg">Simulateur crédit BCEG</h3>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-4 p-1 bg-white/5 rounded-xl">
        <button
          ...
          className={`flex items-center justify-center gap-2 py-2 px-3 rounded-lg text-sm font-medium transition-all ${
            type === 'particulier' ? 'bg-amber-400 text-slate-950 shadow-md' : 'text-white/70 hover:text-white'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center justify-center gap-2 py-2 px-3 rounded-lg text-sm font-medium transition-all ${
            type === 'entreprise' ? 'bg-amber-400 text-slate-950 shadow-md' : 'text-white/70 hover:text-white'
          }`}
        >
      ...
      <div className="space-y-4 mb-4">
        ...
        {type === 'particulier' && (
          <div>
            <label className="flex items-center gap-2 text-xs text-white/80 mb-1">
            ...
            <input
              ...
              className="w-full px-3 py-2 bg-white/10 border border-white/15 rounded-lg text-white placeholder-white/30 text-sm focus:outline-none focus:border-amber-400/50"
            />
          </div>
        )}
      </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {showCapacite && (
              <div className={`col-span-2 rounded-lg px-3 py-2 text-xs font-medium ${
                capaciteOk ? 'bg-emerald-500/15 border border-emerald-400/30 text-emerald-200' : 'bg-red-500/15 border border-red-400/30 text-red-200'
              }`}>
                {capaciteOk
  ...
  return (
    <motion.div
      ...
      className={`rounded-lg px-3 py-2.5 ${
        accent
          ? 'bg-gradient-to-br from-amber-400/20 to-orange-500/15 border border-amber-300/30'
          : 'bg-white/5 border border-white/10'
      }`}
    >
      <div className="flex items-center gap-1 text-[10px] uppercase tracking-wider text-white/60 mb-0.5">
        {icon}
        {label}
      </div>
      <div className={`font-bold tabular-nums ${accent ? 'text-amber-200 text-base sm:text-lg' : 'text-white text-sm'}`}>
        {value}
      </div>
    </motion.div>
  )
