...
exports.handler = async (event, context) => {
  ...
  try {
    const { title, subtitle, google_maps_url, display_order = 0 } = JSON.parse(event.body)

    // Validation des données
    if (!title || !google_maps_url) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ 
          success: false, 
          error: 'Titre et URL Google Maps requis' 
        })
      }
    }

    // Convertir l'URL Google Maps en URL embed
    const embedUrl = convertToEmbedUrl(google_maps_url)
    ...
    const { data: route, error } = await supabase
      ...
      .insert({
        ...
        google_maps_url,
        embed_url: embedUrl,
        html_content: null, // Pas de html_content via ce endpoint simplifié pour l'instant
        display_order,
        is_active: true,
        created_by: null
      })
...
function convertToEmbedUrl(url) {
  ...
  try {
    ...
    // Si aucun pattern ne correspond, retourner l'URL originale au lieu d'une carte par défaut
    // Cela permet à l'utilisateur de corriger l'URL ou d'utiliser une URL qui fonctionne déjà
