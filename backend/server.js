...
// ============================================
// FOOTBALL SCORES API - LIVE MATCHES
// ============================================

// GET: Récupérer les matchs de football en direct via RapidAPI
app.get('/api/football/fixtures', async (req, res) => {
  try {
    console.log(`⚽ [LIVE] Récupération matchs en direct...`);

    // Clé API depuis variables d'environnement uniquement (sécurité)
    const apiKey = process.env.RAPIDAPI_KEY || process.env.RAPIDAPI_FOOTBALL_KEY;
    
    if (!apiKey) {
      console.warn('⚠️ Aucune clé API Football trouvée (ni .env ni fallback)');
      return res.status(503).json({
        success: false,
        error: 'Service Football non configuré (RAPIDAPI_KEY manquant)'
      });
    }
    ...
    if (data.response && Array.isArray(data.response)) {
      data.response = data.response.filter(match => {
        ...
        const leagueType = match.league.type; // 'League' ou 'Cup'
        const country = match.league.country;

        // 1. Compétitions majeures par ID
        if (ALLOWED_LEAGUE_IDS.includes(leagueId)) return true;

        // 2. Sélections nationales (souvent country = 'World' ou type = 'Cup' avec noms spécifiques)
        if (country === 'World' || match.teams.home.national === true || match.teams.away.national === true) {
            return true;
        }
    ...
    console.log(`✅ ${data.response?.length || 0} matchs en direct récupérés (filtrés)`);
